...
# User operations
def get_or_create_user(user_id: int, full_name: str, username: Optional[str] = None) -> User:
...
def get_or_create_player_stats(user_id: int, chat_id: int) -> Dict[str, Any]:
    ...
    with get_db_session() as session:
        ...
        if not stats:
            ...
            config = get_config()
            initial_score = config.get('user', 'new_user_bonus', 500)
...
def get_recent_games(chat_id: int, limit: int = 10) -> List[Game]:
...
def create_bet(game_id: int, user_id: int, bet_type: str, amount: int, 
               referral_points_used: int = 0) -> Bet:
    """Create a new bet."""
    with get_db_session() as session:
        bet = Bet(
            game_id=game_id,
...
def get_game_bets(game_id: int) -> List[Bet]:
    """Get all bets for a game."""
    with get_db_session() as session:
        return session.query(Bet).filter(Bet.game_id == game_id).all()
...
# Admin operations
def get_or_create_admin_data(user_id: int, chat_id: int) -> AdminData:
...
def update_admin_points(user_id: int, chat_id: int, points: int) -> bool:
    """Update admin points."""
    with get_db_session() as session:
        admin_data = get_or_create_admin_data(user_id, chat_id)
